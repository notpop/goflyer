...
const (
	SIGNAL_EVENT_SIDE_SELL = "SELL"
	SIGNAL_EVENT_SIDE_BUY  = "BUY"
)
...
func GetSignalEventsByCount(loadEvents int) *SignalEvents {
	cmd := fmt.Sprintf(`SELECT * FROM (
        ...
        ORDER BY time ASC;`, TABLE_NAME_SIGNAL_EVENTS)
...
func (s *SignalEvents) CanBuy(time time.Time) bool {
	...
	if lastSignal.Side == SIGNAL_EVENT_SIDE_SELL && lastSignal.Time.Before(time) {
		return true
	}
	return false
}
...
func (s *SignalEvents) CanSell(time time.Time) bool {
	...
	lastSignal := s.Signals[lenSignals-1]
	if lastSignal.Side == SIGNAL_EVENT_SIDE_BUY && lastSignal.Time.Before(time) {
		return true
	}
	return false
}
...
func (s *SignalEvents) Profit() float64 {
	...
	for i, signalEvent := range s.Signals {
		if i == 0 && signalEvent.Side == SIGNAL_EVENT_SIDE_SELL {
			continue
		}
		if signalEvent.Side == SIGNAL_EVENT_SIDE_BUY {
			total -= signalEvent.Price * signalEvent.Size
			isHolding = true
		}
		if signalEvent.Side == SIGNAL_EVENT_SIDE_SELL {
			total += signalEvent.Price * signalEvent.Size
			isHolding = false
			beforeSell = total
		}
